            ...
            <div id="demo-chat-body" class="collapse in">
                <div class="nano has-scrollbar" style="height:380px">
                    <div class="nano-content pad-all" tabindex="0" style="right: -17px;">
                        <ul class="list-unstyled media-block" id="list-message">                         
                        </ul>
                    </div>
                ...
                <div class="panel-footer">
                    <div class="row">
                        <div class="col-md-9">
                            <input type="text" placeholder="Enter your text" class="form-control chat-input" id="textMessage">
                        </div>
                        <div class="col-xs-3">
                            <button class="btn btn-primary btn-block" id="submitMessage" >Send</button>
                        </div>
                    </div>
                </div>
            </div>
...
<script type="text/javascript">
    ...
    $('#submitMessage').on('click', function(e){    
        ...
        $.ajax({
          method: "POST",
          url: "{{ url('chat') }}",
          data: { user_id: 1, messageTo_id: "{{ $to }}", name: "{{ $nama }}",  text: text, _token: "{{ csrf_token() }}" }
        })
        .success(function( response ) {
            //console.log('response: ', response);
            $('#list-message').append(template);
        })
...
<script>
  var socket = io.connect('http://localhost:9090', { query: {id:"{{ $nama }}"} });

  socket.emit('register', "{{ $nama }}");

  socket.on('connect_error', function(e){
    console.log('socket.on.connection: ', e)
  })
  ...
  socket.on('message', function (data) {
    ...
    var template = '<li class="mar-btm">\
                                ...
                                <div class="media-body pad-hor">\
                                    <div class="speech">\
                                        <a href="#" class="media-heading">'+to+'</a>\
